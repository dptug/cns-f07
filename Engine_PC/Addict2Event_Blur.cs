...
public class Addict2Event_Blur : Addict2Event
{
	...
	{
		Addict2Logger.Append("Rendering Blur at " + DEData.TimePos + "\n");
		...
		RGBA rGBA = new RGBA((byte)Addict2RenderingTools.LinearInterpolate((int)StartCol.R, (int)EndCol.R, DEData.TimePos), (byte)Addict2RenderingTools.LinearInterpolate((int)StartCol.G, (int)EndCol.G, DEData.TimePos), (byte)Addict2RenderingTools.LinearInterpolate((int)StartCol.B, (int)EndCol.B, DEData.TimePos), (byte)Addict2RenderingTools.LinearInterpolate((int)StartCol.A, (int)EndCol.A, DEData.TimePos));
		rGBA.A = (byte)((double)(int)rGBA.A * 0.8);
		...
		for (int i = 0; i < LayerNum; i++)
		{
			...
			Addict2Engine.device.PixelShader = Addict2RenderingTools.pshBlur;
			zero.X = (float)(int)LayerZoom / 255f / 100f * (float)i;
			...
			Addict2RenderingTools.SetColor(rGBA);
